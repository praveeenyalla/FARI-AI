...
import { FariAlLogoIcon, CodeXmlIcon, MessageSquareIcon, PenLineIcon, BrushIcon, SunIcon, MoonIcon, ShareIcon, SettingsIcon, MenuIcon, ArrowLeftToLineIcon, ArrowRightToLineIcon } from '../../components/Icons';
import WebsiteCreatorView from '../../components/WebsiteCreatorView';
...
const ChatWindow: React.FC<ChatWindowProps> = ({ chats, setChats, activeChatId, setActiveChatId, currentModelId, onModelChange, setTasks, onToggleSidebar }) => {
  ...
  const handleSendMessage = async (text: string, attachment?: Attachment, fromUserMessageId?: string, thinkConfig?: ThinkConfig) => {
    ...
    const MAX_MEMORY_CHATS = 2;
    const MAX_MESSAGES_PER_MEMORY_CHAT = 6;
    ...
    try {
      const isImageGenCommand = (prompt: string): boolean => {
        ...
        if (objectWords.some(word => lowerPrompt.startsWith(`${word} of`))) return true;
      ...
      const isImageQueryCommand = (prompt: string): boolean => {
          ...
          if (lowerPrompt === 'what is this?' || lowerPrompt === 'describe this' || lowerPrompt === 'explain this') return true;
          if (actionWords.some(word => lowerPrompt.includes(word)) && objectWords.some(word => lowerPrompt.includes(word))) return true;
        ...
        default: {
          ...
          for await (const chunk of stream) {
            ...
            setChats(prev => prev.map(c => {
                ...
                const updatedMessages = c.messages.map(m => m.id === botMessageId ? { ...m, text: fullResponse } : m);
