...
import { XIcon, MicIcon, PenLineIcon, SettingsIcon, HeadphonesIcon } from '../../components/Icons';
import ChooseVoiceModal from '../../components/ChooseVoiceModal';
...
const VoiceMode: React.FC<VoiceModeProps> = ({ onClose, initialHistory, chatMode, fariModelId }) => {
  ...
  const sendTurnToAI = useCallback(async (text: string) => {
    ...
    if (currentLang === 'te-IN' && (lowerTranscript.includes('switch back to english') || lowerTranscript.includes('english ki maaru'))) {
    ...
    try {
        ...
        for await (const chunk of stream) {
            ...
            setLastBotResponse(fullResponse);
        ...
        const botMessage: Message = { id: uuidv4(), text: fullResponse, sender: 'bot', timestamp: new Date().toISOString() };
